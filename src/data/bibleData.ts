...
// Full list of Catholic Canon books
export const books: Book[] = [
  ...
  { id: 'numbers', name: 'Numbers', testament: 'OT', chapters: 35 },
  { id: 'deuteronomy', name: 'Deuteronomy', testament: 'OT', chapters: 33 },
  ...
  { id: '1samuel', name: '1 Samuel', testament: 'OT', chapters: 29 },
  { id: '2samuel', name: '2 Samuel', testament: 'OT', chapters: 23 },
  { id: '1kings', name: '1 Kings', testament: 'OT', chapters: 22 },
  { id: '2kings', name: '2 Kings', testament: 'OT', chapters: 23 },
  ...
  { id: 'nehemiah', name: 'Nehemiah', testament: 'OT', chapters: 12 },
  { id: 'tobit', name: 'Tobit', testament: 'OT', chapters: 12 },
  { id: 'judith', name: 'Judith', testament: 'OT', chapters: 15 },
  { id: 'esther', name: 'Esther', testament: 'OT', chapters: 15 },
  { id: '1maccabees', name: '1 Maccabees', testament: 'OT', chapters: 16 },
  { id: '2maccabees', name: '2 Maccabees', testament: 'OT', chapters: 15 },
  { id: 'job', name: 'Job', testament: 'OT', chapters: 42 },
  { id: 'psalms', name: 'Psalms', testament: 'OT', chapters: 149 },
  ...
  { id: 'wisdomofsolomon', name: 'Wisdom of Solomon', testament: 'OT', chapters: 19 },
  { id: 'sirach', name: 'Sirach', testament: 'OT', chapters: 51 },
  ...
  { id: 'baruch', name: 'Baruch', testament: 'OT', chapters: 5 },
  ...
  { id: 'haggai', name: 'Haggai', testament: 'OT', chapters: 1 },
  ...
  { id: 'john', name: 'John', testament: 'NT', chapters: 20 },
  ...
  { id: '2corinthians', name: '2 Corinthians', testament: 'NT', chapters: 12 },
  ...
  { id: '1john', name: '1 John', testament: 'NT', chapters: 4 },
  ...
  { id: 'revelation', name: 'Revelation', testament: 'NT', chapters: 21 },
];

let cachedBibleData: any = null;

export const getChapterText = async (bookId: string, chapterNum: number): Promise<Verse[]> => {
  // Fetch the JSON if not already loaded
  if (!cachedBibleData) {
    try {
      const response = await fetch('/nrsv.json');
      cachedBibleData = await response.json();
    } catch (e) {
      console.error("Failed to load NRSV data", e);
      return [{ chapter: chapterNum, verse: 1, text: "Error loading Bible text." }];
    }
  }

  // The bookId from our books array is short ('gen', 'exo').
  // Let's map it to the full name which the python script used.
  const book = books.find(b => b.id === bookId);
  if (!book) return [];

  const bookName = book.name;
  
  if (cachedBibleData && cachedBibleData.verses && cachedBibleData.verses[bookName]) {
    const chapterData = cachedBibleData.verses[bookName][chapterNum.toString()];
    if (chapterData) {
      return chapterData;
    }
  }
  ...
  return [
    { chapter: chapterNum, verse: 1, text: `Text for ${bookName} Chapter ${chapterNum} not found in the parsed PDF data.` }
  ];
};
